...
export default async function Home() {
  ...
  return (
    <div className="p-6 space-y-6 bg-[#04070d] min-h-screen bg-tactical-grid">
      {/* ROW 1: Hero & Next Tournament */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        {/* Hero Banner (Tactical Chamfered with HUD accents) */}
        <div className="xl:col-span-2 relative overflow-hidden bg-gradient-to-r from-[#001736] via-[#090f1d] to-[#04070d] border border-blue-900/60 p-8 flex flex-col justify-between min-h-[340px] clip-chamfer-tr shadow-2xl hud-box hud-bracket-cyan">
          ...
          <div className="relative z-10 w-full md:w-2/3 space-y-3">
            <div className="inline-flex items-center gap-2 bg-yellow-400 text-slate-950 px-3 py-1 text-[10px] font-black tracking-widest uppercase clip-tag-angled shadow">
            ...
            <div className="pt-4 flex flex-wrap gap-3">
              <Link
                href="/torneos"
                className="bg-yellow-400 hover:bg-yellow-500 text-slate-950 font-black text-xs px-6 py-3 transition-all tracking-widest clip-btn-tactical shadow-lg shadow-yellow-400/20 hover:scale-105"
              >
                VER PRÓXIMOS TORNEOS
              </Link>
              <Link
                href="/decks"
                className="bg-[#0b1222] hover:bg-slate-800 text-white font-black text-xs px-6 py-3 border border-slate-700 hover:border-slate-500 transition-all tracking-widest clip-btn-tactical"
              >
                EXPLORAR TOP DECKS
              </Link>
            </div>
        ...
        <div className="bg-[#070b14] border border-slate-800 p-6 flex flex-col justify-between relative shadow-xl clip-chamfer-tr hud-box hud-bracket-yellow">
          <div>
            <div className="flex justify-between items-center mb-4">
              ...
              {nextTournament && (
                <span className="text-[9px] font-black px-2.5 py-0.5 bg-yellow-400/10 border border-yellow-400/30 text-yellow-400 uppercase clip-tag-angled">
            ...
            {nextTournament ? (
              <div className="space-y-4">
                <div>
                  <h3 className="text-2xl font-black italic text-white line-clamp-2 leading-tight">
                    {nextTournament.name}
                  </h3>
                  <p className="text-xs text-yellow-400 font-bold mt-1">Premio: {nextTournament.prize || "Por definir"}</p>
                </div>

                <div className="space-y-2 text-xs font-semibold text-slate-300 bg-[#0c1220] p-3.5 border border-slate-800">
          ...
          <Link
            href="/torneos"
            className="w-full text-center bg-yellow-400 hover:bg-yellow-500 text-slate-950 font-black py-3 text-xs transition-colors tracking-widest mt-6 clip-btn-tactical shadow-lg shadow-yellow-400/20"
          >
            VER CALENDARIO
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        {/* Ultimo Torneo */}
        <div className="xl:col-span-4 bg-[#070b14] border border-slate-800 p-6 flex flex-col justify-between shadow-xl clip-chamfer-tr">
          <div>
              ...
              <div className="space-y-4">
                <div className="flex gap-4 items-center">
                  <div className="w-24 h-28 bg-gradient-to-br from-yellow-400/20 via-slate-900 to-slate-950 border border-yellow-400/40 clip-chamfer-tr flex flex-col items-center justify-center text-center p-2 shrink-0 shadow">
                    ...
                    <span className="text-[8px] text-yellow-400 font-black uppercase">CAMPEÓN</span>
                    <span className="text-xs font-black text-white truncate max-w-full">
                      {recentTournament.decklists[0]?.playerName?.toUpperCase() ?? "—"}
                    </span>
                  </div>
                  <div className="min-w-0">
                    <span className="text-[9px] font-black px-2 py-0.5 bg-slate-800 text-yellow-400 uppercase clip-tag-angled">
                ...
                <div className="space-y-1.5 text-xs font-semibold text-slate-300 bg-[#0c1220] p-3 border border-slate-800">
                  {recentTournament.decklists.slice(0, 4).map((d, i) => (
                    <div key={d.id} className="flex items-center justify-between">
                      <div className="flex items-center gap-2 truncate">
                        <span className={`w-4 h-4 rounded-sm flex items-center justify-center text-[8px] font-black ${
                      ...
                      <span className="text-[10px] text-slate-400 font-semibold truncate max-w-[120px]">{d.deckName || "Deck"}</span>
                    </div>
                  ))}
          ...
          <div className="flex gap-2 mt-4 pt-3 border-t border-slate-800">
            <Link href="/torneos" className="flex-1 text-center bg-[#0c1220] hover:bg-slate-800 border border-slate-700 text-white text-xs font-black py-2.5 transition-colors clip-btn-tactical">
              RESULTADOS
            </Link>
            <Link href="/decks" className="flex-1 text-center bg-yellow-400 hover:bg-yellow-500 text-slate-950 text-xs font-black py-2.5 transition-colors clip-btn-tactical">
              TOP DECKS
            </Link>
          </div>
        </div>

        {/* Top Decks with TACTICAL CHAMFERED CORNERS and COVER ART */}
        <div className="xl:col-span-5 bg-[#070b14] border border-slate-800 p-6 flex flex-col justify-between shadow-xl clip-chamfer-tr hud-box hud-bracket-cyan">
          <div>
            <div className="flex justify-between items-center mb-4">
                ...
                {topDecks.map((deck, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-full aspect-[3/4] bg-[#0c1220] border border-slate-700 group-hover:border-yellow-400 transition-all mb-2 relative overflow-hidden flex items-center justify-center shadow-lg clip-chamfer-tr">
                        {cover ? (
                          <img
                            src={cover}
                          ...
                          <div className="text-center p-2 text-slate-600">
                            <span className="text-[10px] font-black block">TCG</span>
                            <span className="text-[8px] font-bold">CARD</span>
                          </div>
                        )}
                        {/* Position Badge with Angular Tag */}
                        <div className={`absolute top-0 left-0 px-2 py-0.5 font-black text-[10px] shadow clip-tag-angled ${
                          i === 0 ? "bg-yellow-400 text-slate-950" : i === 1 ? "bg-slate-300 text-slate-950" : "bg-amber-600 text-white"
                        }`}>
                          #{i + 1}
                        </div>
                      ...
                      <span className="text-[9px] text-slate-400 font-bold">TOP {deck.placement}</span>
                    </Link>
                  );
                })}
              </div>
        ...
        {/* Ranking Preview with TACTICAL HUD */}
        <div className="xl:col-span-3 bg-[#070b14] border border-slate-800 p-6 flex flex-col justify-between shadow-xl clip-chamfer-tr">
          <div>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-[10px] font-black text-slate-400 tracking-widest uppercase">RANKING</h3>
...
async function RankingPreview() {
  ...
  return (
    <div className="space-y-3">
      {sorted.map((p, i) => (
        <div key={p.name} className="flex items-center justify-between py-1 border-b border-slate-900/60 pb-1.5">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className={`font-black text-xs w-5 ${colors[i]}`}>{String(i + 1).padStart(2, "0")}</span>
            {/* Player Avatar / Deck Cover Thumbnail */}
            <div className="w-8 h-8 rounded-sm bg-[#0c1220] border border-slate-700 overflow-hidden flex items-center justify-center shrink-0 shadow">
              {p.coverUrl ? (
